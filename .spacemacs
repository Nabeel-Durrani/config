...
(defvar-local ndu/popup-origin-buffer nil
  "Stores the original buffer from which the popup was opened.")
(defvar-local ndu/popup-start-pos nil
  "Stores the starting character position of the original point line.")
(defvar-local ndu/popup-orig-len nil
  "Stores the original character length of the point text.")
(defun ndu/org-save-and-close-popup ()
  "Save the edited text back to the original continuous line and close the popup window."
  (interactive)
  (let ((edited-text (buffer-substring-no-properties (point-min) (point-max)))
        (target-buf ndu/popup-origin-buffer)
        (start-pos ndu/popup-start-pos)
        (orig-len ndu/popup-orig-len))
    (if (and target-buf (buffer-live-p target-buf))
        (with-current-buffer target-buf
          (save-excursion
            (goto-char start-pos)
            (delete-region start-pos (+ start-pos orig-len))
            ;; Unwrap text back into a single continuous line
            (let ((single-line (replace-regexp-in-string "\n" " " edited-text)))
              (insert single-line)
              ;; Wipe out double spaces generated by the unwrapping process
              (goto-char start-pos)
              (while (search-forward "  " (line-end-position) t)
                (replace-match " ")))))
      (message "Warning: Original buffer is no longer available."))
    ;; Close the window and delete the temporary buffer
    (quit-window t)))
(defun ndu/org-edit-point-in-popup ()
  "Edit the current Org dot point in a temporary filled window split."
  (interactive)
  (let* ((orig-buf (current-buffer))
         (beg (line-beginning-position))
         (end (line-end-position))
         (text (buffer-substring-no-properties beg end))
         (pop-buffer (get-buffer-create "*Org Point Edit*")))
    (with-current-buffer pop-buffer
      ...
      (goto-char (point-min))
      ;; Assign the persistent data values directly to the buffer's variables
      (setq ndu/popup-origin-buffer orig-buf)
      (setq ndu/popup-start-pos beg)
      (setq ndu/popup-orig-len (length text))
      ;; Explicitly map the hotkey to our named saving function
      (local-set-key (kbd "C-c C-c") #'ndu/org-save-and-close-popup))
    ;; Force split the layout and shift the focus over to it
    (display-buffer pop-buffer '(display-buffer-pop-up-window))
    (select-window (get-buffer-window pop-buffer))))
...
(defun ndu/org-mode ()
  ...
  (setq org-list-demote-modify-bullet
        '(("*" . "+")   ; Level 1 (-) turns into Level 2 (+) on indent
          ...
          ("1." . "a.")))
  (with-eval-after-load 'org
    (define-key org-mode-map (kbd "C-c e") #'ndu/org-edit-point-in-popup)))
(defun ndu/latex ()
  (require 'ox-latex)
